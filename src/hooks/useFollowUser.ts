...
const useFollowUser = (initialUserId: string) => {
  ...
  const fetchFollowStatus = useCallback(async (userId: string) => {
  ...
  const handleFollowUserWithId = useCallback(async (userId: string) => {
    if (!authUser) {
      navigate("/auth");
      return;
    }
  ...
  }, [authUser, navigate, setAuthUser, setUserProfile, showError, fetchFollowStatus]);
  ...
  return { isUpdating, isFollowing, handleFollowUser, handleFollowUserWithId ,fetchFollowStatus};
};
